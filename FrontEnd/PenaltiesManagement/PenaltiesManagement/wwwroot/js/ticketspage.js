...
$(document).ready(function () {
    ...
    $('#TransactionDateFrom').datepicker({
        ...
        onSelect: function (selectedDate) {
            $('#RegisteredTo').datepicker('option', 'minDate', selectedDate);
        }
    });
    ...
    $('#TransactionDateTo').datepicker({
        ...
        nextText: '<i class="fa fa-chevron-right"></i>',
        onSelect: function (selectedDate) {
            $('#RegisteredFrom').datepicker('option', 'maxDate', selectedDate);
        }
    });
    ...
    $("#ticketDetails").on('submit', function (e) {
        var form = $(this);
        if (form.valid()) {
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    console.log(data); // show response
                    if (data.errorCode === 0) {
                        $('#ticketDetails_success').dialog('open');
                        setTimeout(function () {
